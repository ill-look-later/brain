...
(declare start-class end-class start-main end-main convert-insns)
...
(defn convert-insns [ir mw]
  (if (empty? ir)
    ...
    (let [[insn & insns] ir]
      (do (case (:op insn)
            mc (.visitIincInsn mw 2 (:val insn))
            mv (doto mw
